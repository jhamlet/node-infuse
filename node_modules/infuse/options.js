...
var 
    ...
    /*globals getPluginsByNameForType */
    minifiers   = getPluginsByNameForType('minify'),
    processors  = getPluginsByNameForType('processor'),
    definitions,
    program,
    options
;
...
// Utility Functions
//---------------------------------------------------------------------------
function getPluginsByNameForType (type) {
    return plugins.getType(type).map(function (plugin) {
        return plugin.id;
    });
}
...
program = require('commander').
    ...
    option(
        ...
        function (arg) {
            var resolved = resolve(arg, process.cwd()),
    ...
    option(
        '-m, --minify <NAME>',
        format('minify output using NAME minifier'),
        function (arg) {
    ...
    option(
        '--minify-opts <ARGS>',
        'string of arguments to pass on to the minify plugin'
    ).
    option(
        '-n, --nodelib <PATH>',
...
program.on('--help', function () {
    ...
    ['',
     ...
     '',
     'Supported minifiers: ' + minifiers.join(', '),
     'Supported pre-processor file extensions: ' + processors.map(function (p) {
         return '.' + p;
     }).join(', '),
     ''
...
// Required Arguments
//---------------------------------------------------------------------------
if (!options.infile && !(options.modules && options.modules.length)) {
    console.error('');
