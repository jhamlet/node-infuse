...
module.exports = Proteus.Class.derive(Object.defineProperties({
     ...
     * 
     * @param {String} id optional infusion id to use
     * @param {Object} infusion
     *      @property {Object} infusion.rules
     *      @property {Object} infusion.events
     * @returns {String|Number} the added infusions id
     */
    use: function (id, infusion) {
        var infusions = this.infusions,
            idMap     = this.__idMap__;
        
        if (!infusion && typeof id !== 'string') {
            infusion = id;
            id = infusions.length;
        }
        else if (!infusion) {
            throw new Error('Infuser: an infusion must be an object.');
        }
        
        if (idMap[id]) {
            throw new Error('Infuser: infusion id \'' + id + '\' already defined.');
        }
    ...
    /**
     * Get an infusion by id or index
     */
    get: function (id) {
        if (typeof id === 'number') {
            return this.infusions[id];
        }
        
        return this.infusionsIdMap[id];
    },
    /**
    ...
    leave: (function () {
        ...
        var currNode, nodeUpdate, modified;
        // if the node is modified, we want to revisit that node
        function tempUpdate () {
            nodeUpdate.apply(currNode, arguments);
            modified = arguments.length > 0;
        }
        
        return function (node) {
            currNode   = node;
            nodeUpdate = currNode.update;
            modified   = false;
            currNode.update = tempUpdate;
            // allow event handlers to modify the node
            this.notify('leave', currNode);
            // before executing our infusions
            this.exec(currNode);
            // revert to the nodes regular update method
            currNode.update = nodeUpdate;
