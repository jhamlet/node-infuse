...
var Proteus = require("proteus"),
    ...
    FS      = require("fs"),
    Path    = require('path'),
    plugins = require('infuse/plugins')
;

function preProcess (filepath, src) {
    var processor;
    
    processor = plugins.getType('processor').filter(function (proc) {
        return proc.willRun(filepath);
    })[0];
    
    src = src || FS.readFileSync(filepath, 'utf8');

    return processor ? processor.run(src) : src;
}
/**
 ...
 * and "trailingComments" on individual nodes in the AST returned.
 * 
 * @function parse
