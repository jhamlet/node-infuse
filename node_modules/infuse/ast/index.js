
var Proteus     = require("proteus"),
    parse       = require("./parse"),
    stringify   = require("./stringify"),
    traverse    = require("./traverse"),
    nodeWrapper = require("./node-wrapper")
;

module.exports = Proteus.Class.derive(Object.defineProperties({
    
    self: {
        NODE_TYPES:         traverse.NODE_TYPES,
        NODE_CHILD_KEYS:    traverse.NODE_CHILD_KEYS,
        TRAVERSAL_OPTIONS:  traverse.OPTIONS
    },
    
    init: function (src, opts) {
        var parserOpts, genOpts, subj;
        
        if (arguments.length < 2 && typeof src === "object") {
            opts = src;
            src = null;
        }
        // A little options juggling
        opts = opts || {};
        parserOpts = opts.parser || {};
        genOpts = opts.generator || {};
        delete opts.parser;
        delete opts.generator;
        
        this.options = Proteus.merge({}, this.options, {
            parse: parserOpts,
            generate: genOpts,
            source: src
        }, opts);

        this.reset();
    },
    /**
     * @property {Object} options
     */
    options: {
        /**
         * Options to pass onto the code parser
         * 
         * @property {Object} options.parse
         */
        /**
         * Options to pass onto the code generator
         * 
         * @property {Object} options.generate
         */
    },
    
    traverse: function (visitor) {
        traverse(this.subject, visitor);
        return this;
    },
    
    reset: function () {
        /**
         * @property {Boolean} parsed
         */
        /**
         * @property {Boolean} generated
         */
        this.generated = this.parsed = false;
        return this;
    }
}, {
    /**
     * @property {Object} subject
     */
    subject: {
        get: function () {
            var opts = this.options,
                parserOpts = opts.parse
            ;
            
            if (!this.parsed) {
                parserOpts.source = opts.source;
                parserOpts.file = opts.file;
                opts.ast = nodeWrapper(parse(parserOpts));
                this.parsed = true;
            }
            
            return opts.ast;
        },
        set: function (subj) {
            this.options.ast = subj;
            this.parsed = true;
            this.generated = false;
        },
        enumerable: true
    },
    /**
     * @property {String} source
     */
    source: {
        get: function () {
            var opts = this.options,
                genOpts = opts.generate
            ;
            
            if (!this.generated) {
                opts.source = stringify(this.subject, genOpts);
                this.generated = true;
            }
            
            return opts.source;
        },
        set: function (src) {
            var opts = this.options;
            
            opts.source = src;
            this.parsed = false;
            this.generated = true;
        },
        enumerable: true
    },
    
    file: {
        get: function () {
            return this.options.file;
        },
        set: function (f) {
            var opts = this.options;
            opts.file = f;
            delete opts.source;
            this.parsed = false;
            this.generated = false;
        },
        enumerable: true
    },
    
    valueOf: { value: function () {
        return this.subject;
    } },
    
    toString: { value: function () {
        return this.source;
    } }
}));